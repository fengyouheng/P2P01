	...
	<head>
		...
		$(function(){
			...
			$("#uploadBtn1").uploadifive({
                ...
                buttonText:'上传正面',  <!--图片文本-->
                multi:false,        <!--不允许同时上传多张-->
                fileType:'image',	<!--限制图片上传类型-->
                fileObjName:'image', <!--接收前台出过来的参数,必须要对应-->
                onUploadComplete : function(file, data) {  <!--data 上传完成后,服务器返回的名字-->
					$("#uploadImg1").prop("src",data);	<!--将data设置给src,方便查找-->	
					$("#uploadImage1").val(data);
                },
                onInit:function(){
					console.log($(".uploadifive-queue-item"));
					$(".uploadifive-queue").hide();  <!--隐藏提示框-->
				}
			});
